...
```
## Progress:
(All unlabeled should be assumed as not ready yet. All are possibly subject to change)
```
__pycache__ files are auto generated by python interpreter, I haven't included them in every folder but they're in ever one almost.
...
├── config.py ✔
├── requirements.txt ✔
├── .env (Private information, an example env has been given)
├── .env.example ✔
├── .gitignore ✔
│
...
│   ├── settings.py
|   ├── trends.py
│   └── __pycache_  (Also auto generated by python)
|
├── templates/
│   ├── base.html
│   ├── dashboard.html
...
│   ├── css/
│   ├── js/
│   └── images/grpahs  (Generated in the back end for selected stock)
│
├── utils/
│   ├── helpers.py
│   └── validation.py
